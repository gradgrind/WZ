...
_YEAR_MISMATCH = "In der gewählten Tabelle ist das Schuljahr falsch"
_CLASS_MISMATCH = "In der gewählten Tabelle ist die Klasse falsch"
...
#
    def year (self):
        return (int(self._schoolyear) if (self._month < self._firstmonth
                    or self._firstmonth == 1)
                else (int(self._schoolyear) - 1))
#
...
class AttendanceTable:
    ...
    def makeAttendanceTable (cls, schoolyear, klass, oldsheet = None):
        REPORT('INFO', _MAKE_ATTENDANCE_TABLE.format(klass = klass))
        ...
        if oldsheet:
            ...
            newsheet = at.save()
            REPORT('INFO', _NEW_TABLE.format(klass = klass))
        return newsheet
#
...
#
    def setupSheet(self):
        ...
        if startyear_cell:
            self._table.setCell(startyear_cell, str(int(self._year) - 1))
        self._table.setCell(ATTENDANCE.attendance_cell_class, self._class)
        # Month sheet
        ...
        self._table.setCell(ATTENDANCE.attendance_cell_classM,
                "Klasse %s" % self._class, sheet = self._wsMonth)
...
#
    def copy_old_sheet(self, filepath):
        ...
        year = table.getABValue(ATTENDANCE.attendance_cell_date2)
        if year != self._year:
            REPORT('ERROR', _YEAR_MISMATCH)
            return False
        klass = table.getABValue(ATTENDANCE.attendance_cell_class)
        if klass != self._class:
            REPORT('ERROR', _CLASS_MISMATCH)
            return False
...
#    quit(0)
    ...
    _klass = '11'
    table = AttendanceTable.makeAttendanceTable(_year, klass = _klass)
    fdir = os.path.join(DATA, 'testing', 'tmp')
    os.makedirs(fdir, exist_ok = True)
    fpath = os.path.join(fdir, 'Attendance_%s.xlsx' % _klass)
