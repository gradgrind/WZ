...
"""
core/pupils_old.py - last updated 2021-02-06

Database access for reading pupil data. OLD FORMAT!
...
USE_XLSX = False
from core.base import Dates, sortingName
...
#
    ...
    TABLE_NAME = 'Klassen0/KLASSE_{year}_{klass}'
#
    def classes(self):
...
#TODO: suggested change to eliminate 'CLASS' delta
        ...
        if changes == None:
            ...
            for klass, dlist in changes.items():
                try:
                    ...
                    cmap[klass] = kchanges
                for d in dlist:
#TODO: 'CLASS' should no longer be possible!
                    if d[0] == 'CLASS':
                        pdata = d[1]
...
#
    def compare_update(self, newclasses):
        ...
        return changes
#
    def update_class(self, klass, changes):
        """Apply the changes in the <changes> list to the pupil data.
        The entries are basically those generated by <self.compare_new_data>,
        but it would be possible to insert a filtering step in between.
        """
        try:
        ...
        # Alphabetical sort
        if pdlist:
            pdlist.sort(key = sortkey)
        for pd in pdlist:
            print(pd)


        return
#TODO: backup and save
        # Save old files as zip-archive
        files = glob.glob(self.read_class_path('*'))
        bakdir = self.read_class_path()
        if files:
...
#----------------------------------------------------------#
    ...
    def save(self):
        ...
        """
        CLASS_TABLE = 'Klassen0/Schueler'
        filepath = year_path(self.schoolyear, CLASS_TABLE)
        ...
        if os.path.isfile(fpath):
            ...
            bpath = os.path.join(filepath, today + '.json.gz')
